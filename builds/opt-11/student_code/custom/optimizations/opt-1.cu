...
__global__ void conv_forward_kernel(float *y, const float *x, const float *k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    auto k_shared = T;
    auto x_shared = T + K * K;
    const auto MSIZE_X = TILE_SIZE_X + K - 1;
    const auto MSIZE_Y = TILE_SIZE_Y + K - 1;
...
#define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    float acc = 0;
    for (int c = 0; c < C; ++c) {
        x_shared[ty * MSIZE_X + tx] = h < H && w < W ? x4d(b, c, h, w) : 0;
        if (tx < K && ty < K) k_shared[ty * K + tx] = k4d(m, c, ty, tx);
        __syncthreads();
        if (tx < TILE_SIZE_X && ty < TILE_SIZE_Y && h < H_out && w < W_out) {
            for(int p = 0; p < K; p++)
                for (int q = 0; q < K; q++)
                    acc += k_shared[p * K + q] * x_shared[(ty + p) * MSIZE_X + (tx + q)];
        }
        __syncthreads();
    }
    if (tx < TILE_SIZE_X && ty < TILE_SIZE_Y && h < H_out && w < W_out && b < B && m < M) {
        y4d(b, m, h, w) = acc;
    }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_y, const float *device_x, const float *device_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    int Y = W_grid * H_grid;
    auto MSIZE_X = TILE_SIZE_X + K - 1;
    auto MSIZE_Y = TILE_SIZE_Y + K - 1;
    auto sharedmem_size = sizeof(float) * (K * K + MSIZE_X * MSIZE_Y);

    dim3 nt(MSIZE_X, MSIZE_Y);
    dim3 nb(B, Y, M);
