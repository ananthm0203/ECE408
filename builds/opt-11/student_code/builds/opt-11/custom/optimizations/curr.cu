...
__global__ void conv_forward_kernel(float *y, const float *x, const float *k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    const auto PARTSIZE = MSIZE * MSIZE + K * K;
...
#define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    for (int c = 0; c < C; ++c) {
        int T_part = c % 2;
        int offset = T_part * PARTSIZE;
        x_shared[offset + ty * MSIZE + tx] = h < H && w < W ? x4d(b, c, h, w) : 0;
        if (tx < K && ty < K) k_shared[offset + ty * K + tx] = k4d(m, c, ty, tx);
        __syncthreads();
        if (tx < TILE_SIZE && ty < TILE_SIZE && h < H_out && w < W_out) {
            for(int p = 0; p < K; p++)
                for (int q = 0; q < K; q++)
                    acc += k_shared[offset + p * K + q] * x_shared[offset + (ty + p) * MSIZE + (tx + q)];
        }
    }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_y, const float *device_x, const float *device_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    auto sharedmem_size = 2 * sizeof(float) * (K * K + MSIZE * MSIZE);
